...
const AppView = () => {
  ...
  const [
    quantidadeDePedidosEntregue,
    setQuantidadeDePedidosEntregue,
  ] = useState([]);
  const [valorRecebidoEntrega, setValorRecebidoEntrega] =
    useState([]);
  ...
  const [
    itensVisiveisPorPedido,
    setItensVisiveisPorPedido,
  ] = useState({});
  const [
    enderecoVisivelPorPedido,
    setEnderecoVisivelPorPedido,
  ] = useState({});
  ...
  const buscarPedidosRecebidos = async () => {
    try {
      ...
      const pedidosEntreguesSnapshot = await getDocs(
        pedidosEntreguesRef
      );
      const pedidosEntreguesData =
        pedidosEntreguesSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
      ...
      const pedidosFinalizadosSnapshot = await getDocs(
        pedidosFinalizadosRef
      );
      const pedidosFinalizadosData =
        pedidosFinalizadosSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));

      const idsPedidosFinalizadosExcluir =
        pedidosFinalizadosData
      ...
      await Promise.all(
        idsPedidosFinalizadosExcluir.map(
          async (idPedidoFinalizado) => {
      ...
      console.error(
        'Erro ao buscar os pedidos recebidos:',
        error
      );
    }
  };
  ...
  const moverParaPreparo = async (pedido) => {
    try {
      ...
      setListaDePedidos((pedidos) =>
        pedidos.filter((p) => p !== pedido)
      );
    } catch (error) {
      console.error(
        'Erro ao mover o pedido para preparo:',
        error
      );
    }
  };

  const moverParaPedidosFinalizados = async (
    pedidoFinalizado
  ) => {
    try {
      const pedidosFinalizadosRef = collection(
      ...
      console.error(
        'Erro ao mover o pedido finalizado:',
        error
      );
    }
  };

  const moverParaPedidosEntregues = async (
    pedidoFinalizado
  ) => {
    try {
      ...
      const pedidoEmPreparoSnapshot = await getDocs(
        pedidoEmPreparoQuery
      );

      if (!pedidoEmPreparoSnapshot.empty) {
        const pedidoEmPreparoDoc =
          pedidoEmPreparoSnapshot.docs[0];
      ...
      console.error(
        'Erro ao mover o pedido para entregues:',
        error
      );
    }
  };
  const buscarPedidosEmPreparo = async () => {
    try {
      ...
      const pedidosEmPreparoSnapshot = await getDocs(
        pedidosEmPreparoRef
      );
      const pedidosEmPreparoData =
        pedidosEmPreparoSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
      ...
      const idsPedidosEmPreparoExcluir =
        pedidosEmPreparoData
      ...
      await Promise.all(
        idsPedidosEmPreparoExcluir.map(
          async (idPedidoEmPreparo) => {
            const pedidoEmPreparoRef = doc(
              db,
              'PEDIDO EM PREPARO',
              'TELEFONE',
      ...
      const pedidosEmPreparoFiltrados =
        pedidosEmPreparoData.filter(
          (pedidoEmPreparo) =>
            !pedidosFinalizadosData.some(
              (pedidoFinalizado) =>
                pedidoFinalizado.numeroPedido ===
      ...
      console.log(
        'numero do pedido',
        pedidosEmPreparoFiltrados
      );
      setPedidoEmPreparo(pedidosEmPreparoFiltrados);
    } catch (error) {
      console.error(
        'Erro ao buscar pedidos em preparo:',
        error
      );
    }
  };
  ...
  const buscarPedidosFinalizado = async () => {
    try {
      ...
      const querySnapshot = await getDocs(
        pedidosFinalizadoRef
      );
      const pedidosFinalizadoData = querySnapshot.docs.map(
        (doc) => ({
          id: doc.id,
          ...doc.data(),
        })
      ...
      await Promise.all(
        pedidosFinalizadoData.map(
          async (pedidoFinalizado) => {
            const pedidosEntreguesRef = collection(
              db,
              'PEDIDOS ENTREGUES',
              'TELEFONE',
      ...
      console.error(
        'Erro ao buscar pedidos finalizados:',
        error
      );
    }
  };
    ...
    try {
      if (pedidoEmPreparo.length > 0) {
        ...
        setPedidoEmPreparo((pedidosEmPreparo) =>
          pedidosEmPreparo.slice(1)
        );
      }
    } catch (error) {
      console.error(
        'Erro ao processar pedido pronto:',
        error
      );
    }
  };
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const pedidosEntreguesData = snapshot.docs.map(
          (doc) => doc.data()
        );

        const pedidosExcluir = listaDePedidos.filter(
          (pedido) =>
            pedidosEntreguesData.some(
              (pedidoEntregue) =>
                pedidoEntregue.numeroPedido ===
        ...
        pedidosExcluir.forEach(async (pedido) => {
          ...
          const pedidoFinalizadoSnapshot = await getDocs(
            pedidoFinalizadoQuery
          );
  ...
  const calcularSomaTotal = (itens) => {
    return itens.reduce(
      (total, item) => total + item.valorTotalDoProduto,
      0
    );
  };
  ...
  const moverEmPreparoParaCancelados = async (
    pedidoEmPreparo
  ) => {
    try {
      ...
      setPedidoEmPreparo((pedidosEmPreparo) =>
        pedidosEmPreparo.filter(
          (pedido) => pedido !== pedidoEmPreparo
        )
      );
    ...
    >
                          ...
                          pedidoEntregue.itens.length >
                            ...
                            <Box>
                                ...
                                pedidoEntregue.itens.map(
                                  (item, itemIndex) => (
                                    ...
                                    >
                                      ...
                                      0 ? (
                                        console.log(
                                          'nao precisa de espaco se forbebida'
                                        )
                                      ) : (
                                        <br />
                                      )}
                                      {item.adicionais
                                        ...
                                        <>
                                          ...
                                          : R${' '}
                                          {item.valorTotalAdicionais.toFixed(
                                            2
                                          )}
                                          <br />
                                        </>
                              ...
                              pedidoEntregue.DadosPessoais
                                .troco === '0' ? (
                                console.log(
                                  'nao precisou de troco/ta trocado'
                                )
                              ) : (
                                <Typography
                                  style={{
      ...
      >
            ...
            {listaDePedidos.map((pedido, index) => (
              <Box
                ...
                sx={{
                  ...
                  overflow: 'hidden',
                  boxShadow:
                    '2px 0px 10px 1px rgba(0, 0, 0, 0.2)',
                }}
              >
                <Box
                  sx={{
                ...
                {enderecoVisivelPorPedido[
                  ...
                  >
                    ...
                    <br />
                    Bairro:{' '}
                    {pedido.DadosPessoais.endereco.bairro}
                    <br />
                    Casa/Apto:{' '}
                    {pedido.DadosPessoais.endereco.casaApto}
                    <br />
                    CEP: {pedido.DadosPessoais.endereco.cep}
                    <br />
                    Cidade:{' '}
                    {pedido.DadosPessoais.endereco.cidade}
                    <br />
                    Complemento:{' '}
                    {
                      pedido.DadosPessoais.endereco
                        .complemento
                    }
                    <br />
                    Estado:{' '}
                    {pedido.DadosPessoais.endereco.estado}
                  </Typography>
                )}
              </Box>
            ))}
                ...
                >
                  ...
                  >
                    ...
                    <LocalPrintshopRoundedIcon
                      ...
                      variant="outlined"
                      onClick={() =>
                        imprimirPedido(pedidoEmPreparo)
                      }
                    />
                  </Box>
                  <Typography sx={{ pl: 1, pt: 1 }}>
                    <b>Nome :</b>{' '}
                    {pedidoEmPreparo.DadosPessoais.nome}
                    <br />
                    <b>Telefone :</b>{' '}
        ...
        >
          ...
          >
            {pedidoFinalizado.map(
              (pedidoFinalizado, index) => (
                <Box
                  key={index}
                  sx={{
                    mt: 1,
                ...
                >
                  ...
                  >
                    ...
                    <LocalPrintshopRoundedIcon
                      ...
                      variant="outlined"
                      onClick={() =>
                        imprimirPedido(pedidoFinalizado)
                      }
                    />
                  </Box>
                  <Typography sx={{ pl: 1, pt: 1 }}>
                    <b>Nome :</b>{' '}
                    {pedidoFinalizado.DadosPessoais.nome}
                    <br />
                    <b>Telefone :</b>{' '}
                    {
                      pedidoFinalizado.DadosPessoais
