...
const Order = () => {
  ...
  const createWhatsAppMessage = (data) => {
    ...
    if (
      sessionStorageData &&
      formaDeEntregaEscolhida === 'Entrega'
    ) {
      ...
      sessionStorageData.forEach((item, index) => {
        ...
        if (ingredientes.includes('Gelada')) {
          console.log(
            'e bebida entao valor de opcional  nao entra'
          );
        } else {
          if (
            (item.opcionalSelecionado &&
              item.valorOpcional === undefined) ||
            (item.opcionalSelecionado &&
              item.valorOpcional === '') ||
            (item.opcionalSelecionado &&
        ...
        if (
          ...
          message += `Valor total do item: R$ ${item.valor.toFixed(
            2
          )}\n`;
        } else {
          ...
          <Box className="cardDeliveryMethod">
            ...
            >
              ...
              <RadioGroup
                ...
                value={isEntrega}
                onChange={(e) =>
                  setIsEntrega(e.target.value === 'Entrega')
                }
              >
                <Box
                  sx={{
